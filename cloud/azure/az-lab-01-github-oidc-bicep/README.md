...
> **Implementation notes (Appendix B).** (1) The AKS workload-identity issuer is shown as `https://oidc.eks.<region>.azure.com/…` — that is AWS EKS naming; AKS issuers look like `https://<region>.oic.prod-aks.azure.com/<tenant-id>/<guid>/` (read it with `az aks show --query oidcIssuerProfile.issuerUrl`). (2) The script mixes Key Vault **access policies** (`az keyvault set-policy`) with **RBAC** role assignments — a vault uses one model; RBAC is recommended. (3) "Function Invoker" is not a built-in Azure role — VERIFY and replace with a custom role. (4) Azure DevOps workload identity federation subjects are `sc://<org>/<project>/<service-connection-name>`, not a pipeline name, and new service connections use an Entra issuer (`https://login.microsoftonline.com/<tenant-id>/v2.0`) — VERIFY CURRENT PRODUCT BEHAVIOUR. (5) The storage deny policy uses alias `…/enableHttpsTrafficOnly`; the property is `supportsHttpsTrafficOnly`. (6) NSG flow logs can no longer be newly created (since 30 June 2025) and retire on 30 September 2027; use virtual network flow logs.
